...
struct ClientConfig
{
    ...
    std::function<void(
        std::mt19937 &,           // генератор случайных чисел
        double &,                 // первый аргумент задачи
        double &,                 // второй аргумент задачи (0.0 для sin/sqrt)
        double &,                 // ожидаемый результат
        std::function<double()> & // сама задача (лямбда без аргументов)
        )>
        make_task;
};
...
{
    return {
        "sin", 0x51A1u, // name, seed, func
        [](std::mt19937 &rng, double &a, double &b, double &expected, std::function<double()> &task)
        {
            // Генератор равномерного распределения на[-1000, 1000]
            ...
            expected = fun_sin(a);
            // Создаём задачу: лямбда захватывает 'a' по значению
            task = [a]()
            { return fun_sin(a); };
        }};
}
