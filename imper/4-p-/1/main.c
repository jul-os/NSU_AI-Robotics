...
{
    // будем сортировать заранее
    // если массив длины 0 то норм, но меньше не создаем
    if (len < 0)
    {
        return 0;
    }
    ...
    int *dataa = malloc(len * sizeof(int));
    memcpy(dataa, arr, len * sizeof(int));
    qsort(dataa, len, sizeof(int), cmp);
    // проверка что нет повторяющихся элементов
    for (int i = 1; i < len; i++)
    {
        // так как массив уже отсортирован то можно сравнивать втупую
